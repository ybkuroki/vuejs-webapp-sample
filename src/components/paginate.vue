<template>
  <ul :class="containerClass" v-if="!noLiSurround">
    ...
    <li v-for="page in pages" :class="[pageClass, { active: page.selected, disabled: page.disabled }]" :key="page.index">
  ...
  <div :class="containerClass" v-else>
    ...
    <template v-for="page in pages">
      <a v-if="page.disabled" :class="[pageLinkClass, { active: page.selected, disabled: page.disabled }]" tabindex="0" :key="page.index">{{ page.content }}</a>
      <a v-else @click="handlePageSelected(page.index)" @keyup.enter="handlePageSelected(page.index)" :class="[pageLinkClass, { active: page.selected, disabled: page.disabled }]" tabindex="0" :key="page.index">
        {{ page.content }}
      </a>
    </template>
...
<script>
// @see https://github.com/lokyoung/vuejs-paginate/blob/master/src/components/Paginate.vue
...
export default {
  ...
  methods: {
    handlePageSelected(selected) {
      ...
      this.$emit('clickHandler', this.selected + 1)
    },
    setPage(selected) {
      if (this.selected === selected) return
      this.selected = selected
